 ...
 *   post:
 *     summary: 取得綠界付款表單參數（前端自行 submit 跳轉）
 *     tags: [Orders]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
router.post('/:id/ecpay-form', (req, res) => {
  ...
  if (!order.merchant_trade_no) {
    return res.status(400).json({ data: null, error: 'MISSING_TRADE_NO', message: '訂單缺少綠界交易編號' });
  }
  ...
  try {
    const { action, params } = ecpayService.buildAioFormParams({ order, items, baseUrl });
...
router.post('/:id/verify-payment', async (req, res) => {
  ...
  try {
    const result = await ecpayService.queryTradeInfo(order.merchant_trade_no);

...
function migrateOrdersEcpayColumns() {
  ...
  addColumn('merchant_trade_no', 'TEXT');
  addColumn('ecpay_trade_no', 'TEXT');
  addColumn('payment_type', 'TEXT');
  addColumn('paid_at', 'TEXT');
  db.exec(
    'CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_merchant_trade_no ' +
    'ON orders(merchant_trade_no) WHERE merchant_trade_no IS NOT NULL'
  );
}
